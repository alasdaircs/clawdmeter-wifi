...
7. **Touch axis swap/mirror is per-board — and on the 2.16, rotation-aware.** The CST9220 needs `setSwapXY(true)` + `setMirrorXY(true, false)` in `touch_hal_init()`, and `touch_hal_read()` then applies the inverse of the display's `rotate_strip` transform per IMU quadrant **with a +1 quadrant offset** — the swap/mirror config was tuned empirically with the device sitting in IMU quadrant 3, so reported coordinates already carry one quadrant of compensation (hardware-verified: without the offset, touch is uniformly 90° off). Position-sensitive touch (settings slider/switch) breaks without this; tap-anywhere screens never noticed.
...
11. **Swipe navigation rides LVGL's gesture bubbling — three rules.** The one `LV_EVENT_GESTURE` handler lives on the LVGL *screen* object (`screen_gesture_cb` in ui.cpp); it works because `lv_obj.c` only sets `LV_OBJ_FLAG_GESTURE_BUBBLE` on objects **with a parent**, so the bubble chain always terminates at the screen. (a) Any widget where a horizontal drag means something (slider, switch) must `lv_obj_clear_flag(…, LV_OBJ_FLAG_GESTURE_BUBBLE)` or dragging it pages the UI. (b) The gesture handler must call `lv_indev_wait_release()` or the finger-up also fires `LV_EVENT_CLICKED` on whatever was under it (splash toggle, hotspot button, BLE reset). (c) Screen transitions animate via `ui_slide_to()` (two full-screen containers x-sliding in lockstep, 250 ms); `ui_show_screen()` stays instant and calls `slide_abort()` first — never start a slide from code that might run mid-slide.
...
- **Hotspot button is now a toggle (2026-08-14).** The Wi-Fi screen button flips to "Stop Hotspot" while the captive portal is active (label set in `ui_update_wifi_creds()`, which already fires on every portal transition). New `captive_portal_stop()` (routes registered once — `WebServer::stop()` never frees handlers) and `wifi_poller_restart()` (inverse of stop: clears flags/counters, reconnects, polls immediately). Manual stop unlocks nav and disarms the 3-failure auto-fallback until reboot so bad creds can't reopen the portal uninvited. Hardware-verified on the 2.16.
- **Swipe navigation; long-press brightness retired (2026-08-06).** Left/right swipes anywhere cycle the four main screens in either direction with a 250 ms slide (gotcha #11 has the gesture rules); PWR short-press cycles with the same animation. Swipe on splash cycles animations; tap still toggles splash instantly. The PWR long-press brightness ramp is gone — `brightness_cycle()` deleted, slider is the only brightness control; the HAL long-press/release edges remain and are drained in `loop()`. Hardware-verified on the 2.16. `screenshot.sh` now finds pio's penv Python on Windows (`penv/Scripts/python.exe`) — pass the COM port explicitly, e.g. `./screenshot.sh out.png COM4`.
- **Settings screen (2026-08-01).** New `SCREEN_SETTINGS` in the cycle (usage → settings → bluetooth → wifi): touch slider for brightness (live preview while dragging, NVS persist on release, floor 10) and a switch for the reset chime (plays the chime as confirmation when switched on). Brightness NVS moved from preset-index to raw level ("brt_lvl", migrates from "brt_idx"); chime pref extracted to `chime_pref.{h,cpp}`. Settings panels don't bubble clicks (tap inside ≠ cycle screen); PWR long-press and serial toggles re-sync the widgets via `ui_settings_refresh()`.
- **Upstream porting sweep (2026-07-26).** Cherry-picked/adapted from upstream: PWR long-press HAL edges (98a513a — long-press cycles brightness on this fork, short-press keeps cycling screens), NVS brightness (ddb812a), rotation-flash ramp (65e51a0), terracotta splash tint (bd5d45c), session-reset chime chain (9b30727/164d9e4/2f94248/debf6f5; opt-in via NVS `chime on|off` instead of daemon config), 1.8 panel-rev autodetect (4e54363), stale-data Zzz view (72524ef — swaps the usage panels for a sleeping creature after 150s without a valid poll), Enterprise overage accounts (fe7a7ee — header parsing + billing-period math live in wifi_poller.cpp). New: the representative-claim header highlights the binding window's pill in terracotta; 7d-reset is now collected so Weekly shows a real reset time.
